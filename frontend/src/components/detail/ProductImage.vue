...
watch(
  () => props.saleItemImages,
  (v) => mapFromSaleItemImages(v || []),
  {
    immediate: true,
    deep: true,
  }
...
const handleUpload = (e) => {
  ...
  const willTake = validFiles.slice(0, emptyIndexes.length);
  if (validFiles.length > emptyIndexes.length) {
    showAlert('error', `Maximum 4 pictures are allowed.`);
  }
...
const clearSlot = (idx) => {
  slots.value[idx] = {
    position: indexToOneBasedOrder(idx),
    fileName: null,
    url: null,
    file: null,
    filename: null,
...
<template>
    ...
    <div class="flex flex-row lg:flex-col justify-center items-center gap-4 lg:gap-6">
      ...
      >
        ...
        <div
          ...
          class="text-[10px] mt-1 max-w-16 text-center truncate"
          :title="slot.filename || slot.fileName || 'empty slot'"
        >
        ...
        <button
          ...
          :class="`itbms-picture-file${index + 1}-clear cursor-pointer hover:bg-red-600 mt-2`"
  ...
  <div class="mt-4 flex gap-3" v-if="!props.isDetailPage">
    <button
      ...
      @click="
        currentFilesCount >= 4 ? showAlert('error', 'อัปโหลดได้สูงสุด 4 รูป') : triggerUpload()
      "
    >
      + Upload Pictures
    </button>
